...
# ─────────────────────────────────────────────────────────────────
# 필터링 설계 원칙 (v3: 분석 중심 필터링)
#
# 목표: "농협에 대한 분석, 사건, 정책 뉴스"만 전달
#
# 필터링 단계:
#   1단계: 농협 관련 기사인지 확인
#   2단계: 구조적 홍보성 패턴 차단 (행사, 지원, 캠페인 등)
#   3단계: 마케팅 블랙리스트 차단 (출시, 이벤트 등)
#   4단계: 분석/사건/정책 화이트리스트로 통과 (분석, 비리, 규제 등)
#
# 핵심: "분석 기사" 또는 "뉴스 가치 있는 기사"만 전달
# ─────────────────────────────────────────────────────────────────

# 화이트리스트 — 분석/사건/정책 기사만 통과
# (분석: 실적, 통계 | 사건: 비리, 논란 | 정책: 규제, 개정)
WHITELIST_KEYWORDS = [
    # 금융·경영 실적 및 분석
...
def is_relevant(title, summary=''):
    ...
    if any(kw in title for kw in PROMO_KEYWORDS):
        return False
    # 4단계: 화이트리스트 — 분석/사건/정책 기사만 통과
    # (분석 기사, 비리 사건, 규제 정책 등만 통과)
