...
KEYWORDS = [
    ...
    # 거시·계열 보강 (v6.8)
    '가계부채', '메리츠',
    # 업종 통칭·거시 (v6.9) — '은행 대출 연체율'·'금융보험업 대출' 누락 대응
    '은행', '금융보험업',
    # 연준 의장 실명 (v6.9) — FOMC 발언 기사 면밀 추적 (의장 교체 시 갱신)
    '파월', '케빈 워시',
    # 거시·규제 보강 (v6.10) — 美금리·긴축·환율·해외투자·책무구조도
    '긴축', '美 금리', '미국 금리', '환율', '외환', '해외투자', '책무구조도',
    # 신탁사 (v6.12) — '[단독] 우리자산신탁 직원 46억 수수' 누락 대응
    '자산신탁', '부동산신탁', '토지신탁',
    # 국민성장펀드 추적 시리즈 (v6.13) — '[비싼 바람]' 등 정책펀드 기획기사 태그
    '비싼 바람',
...
WHITELIST_KEYWORDS = [
    ...
    # 해외 통화정책 (v6.9) — FOMC·연준은 국내시장 영향 커 면밀 추적
    'FOMC', '연준', '동결',
    # 거시·규제 (v6.10) — 책무구조도/환율/해외투자/긴축
    '책무구조도', '환율', '해외투자', '긴축',
    # 금융·경영 실적 및 분석
    ...
    '횡령', '배임', '비리', '부정', '사기', '불법',
    '투서', '내부고발', '감시', '적발',
    '사고', '사건', '전산장애', '전산 장애', '시스템 장애', '서비스 장애', '거래 장애', '오류', '먹통',
...
STRUCTURAL_PROMO_PATTERNS = [
    # 위촉·상품특판·오픈이노·감사결의·비전클리셰·랭킹·개소·발급 PR (v6.16)
    '위촉',                                     # 위원 위촉 동정 (함영주 딸기엑스포 민간위원장)
    '적금 최고금리',                            # 특판 적금 고금리 PR (법정최고금리·잔액동향은 미차단)
    '오픈이노베이션',                           # 스타트업 협업 PR (토스뱅크)
    '감사역량',                                 # 감사 결의·역량강화 PR (전남농협)
    '박차',                                     # 비전 클리셰 (카카오뱅크 플랫폼 박차)
    '수익률 1위', '점유율 제고',                # 랭킹·점유율 마케팅 (우리은행 DC 1위)
    '고삐 죈다',                                # 경영진 행보 클리셰 (롯데카드 정상호)
    '개소',                                     # 창구·센터·지점 개소 (제주남원농협 동행창구)
    '발급 돌파',                                # 카드·증서 발급 마일스톤 (하나은행 국제학생증)
    # ESG보고서·상생대출·회의개최 PR (v6.15)
    '지속가능경영보고서',                       # ESG 지속가능경영보고서 PR (기후공시 정책은 미차단)
    '협력사에',                                 # 상생/CSR 협력사 대출·지원 PR (IBK 현대기아차 협력사)
    '전략회의',                                 # 내부 전략·결산 회의 개최 PR (제주농협 건전결산전략회의)
    # 안전캠페인·서비스 고도화 PR (v6.14)
    'ZERO 사업장', '안전사고',                  # 산업안전 무재해 캠페인 (농협 안전사고 ZERO)
    '서비스 고도화',                            # 핀테크·앱 서비스 고도화 PR (헥토파이낸셜)
    # ESG 고용·회동·증권리포트·시황snippet·WM PR (v6.13)
    '고용연계', '고용 연계', '취업 지원', '직무교육', '인턴십', '일자리 창출', '지속가능 일자리',
    '경영진과 회동', '협력 논의',                 # MOU성 회동 PR
    '목표가',                                     # 증권사 종목 목표가 리포트
    '주가 상승 중', '주가 하락 중',               # 자동생성 시황 snippet
    '맞춤형 자산관리', '함께 성장',               # 지역사 WM 서비스·비전 클리셰 PR
    # 농산물 소비촉진 캠페인 (v6.12) — '양파 소비 나섰다' 류 지역농협 판촉 PR
    '소비 나섰', '팔아주기',
    ...
    'AI로 지자체',                              # 보험사 AI 솔루션 대외 PR (삼성화재 지자체 위험진단)
    '릴레이', '동참',                           # 캠페인 릴레이·동참 동정 (전북은행 박춘원 불법도박 릴레이)
...
PROMO_KEYWORDS = [
    ...
    '조합장상', '○○상', '우수 선정', '베스트 선정', '1위 선정', '대상 선정', '명가 선정', '선정 영예', '수상 기업', '수상 지점',
...
PRIMARY_INDUSTRY_TOPICS = [
    ...
    '축제', '경진대회', '품평회', '직거래장터',
    '양파', '마늘', '배추',                     # (v6.12) 농산물 판촉 — substring 안전한 것만
]
